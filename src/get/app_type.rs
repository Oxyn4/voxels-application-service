...
pub async fn handle_type_method_call(con: Arc<DBusConnection>, mut ctx: Context, parsed_uuid: Result<Uuid, uuid::Error>, data_directory: Arc<RwLock<Option<PathBuf>>>) -> PhantomData<(String,)> {
...
pub fn add_type_method_to_interface(con: Arc<DBusConnection>, data_directory: Arc<RwLock<Option<PathBuf>>>, b: &mut IfaceBuilder<()>) {
    b.method_with_cr_async("type", ("uuid",), ("type",), move |ctx, _, (uuid,): (String,)|{
        ...
        handle_type_method_call(con.clone(), ctx, parsed_uuid, data_directory.clone())
    });
}